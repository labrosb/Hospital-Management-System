<?php
	...
	$sql = mysql_query("SELECT Id, Start, End, Reason FROM unavailable_staff WHERE Staff_id = $doc_id AND Reason = '5' ") or die("cannot connect to unavailable_staff");
	$cnt=0;
	while($row = mysql_fetch_assoc($sql)) {
		$types[$cnt]['id'] = $row['Id'];
...
//---Βάρδια---// 
	...
	$sql = mysql_query("SELECT Id, Start_date, End_date, Unit_id FROM work_shifts WHERE Staff_id = $doc_id ") or die("cannot connect to unavailable_staff");
	while($row = mysql_fetch_assoc($sql)) {
	//echo $cnt;
		$types[$cnt]['id'] = $row['Id'];
